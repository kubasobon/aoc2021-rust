...
fn main() {
    ...
    println!("Collected {} boards", board_count);
    for b in boards.iter() {
        println!("{}\n", b);
    }

    let mut finished_boards = Vec::<usize>::new();
    'number_loop: for given_num in numbers.iter() {
        for (i, b) in boards.iter_mut().enumerate() {
            ...
            if check_row(&b) || check_col(&b) {
                ...
                println!("finished: {:?}", finished_boards);
                if finished_boards.len() == board_count {
                    println!("THE LAST BINGO!\n{}\n", b);
